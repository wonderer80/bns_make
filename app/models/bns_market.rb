  ...
  class << self
    def item_price(itemName)
      ...
      result = list.map do |e|
        num = e.xpath('div[1]/span[2]')[0].children[0].to_s.strip
        gold = e.xpath('div[2]/div[1]').css('.gold').children[0].to_s.strip
        silver = e.xpath('div[2]/div[1]').css('.silver').children[0].to_s.strip
        bronze = e.xpath('div[2]/div[1]').css('.bronze').children[0].to_s.strip
